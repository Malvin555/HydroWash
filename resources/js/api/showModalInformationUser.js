...
function renderModalInformationUser(response) {
    ...
    modalContent.innerHTML = `
        <div class="overflow-y-auto p-6 space-y-4 flex-1">
            <h2 class="text-xl font-medium text-center text-primary tracking-wide">${response?.serviceType} Information</h2>

            <div>
                <label class="text-sm font-semibold text-primary">Amount Item:</label>
                <div class="bg-secondary text-primary px-4 py-2 mt-1 rounded-md text-sm">${data?.amount_item}Pcs ${new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR" }).format(data.price_ironing ?? data.price_laundry)}</div>
            </div>
            ...
            <div>
                ...
                <div class="bg-secondary text-primary px-4 py-2 mt-1 rounded-md text-sm min-h-25">${data.notes_ironing ?? data.notes_laundry ?? 'Nothing'}</div>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-2 bg-secondary text-primary px-4 py-2 mt-1 rounded-md text-sm">
                    ...
                    <span>${data.estimation ?? 'Null Pay first'}</span>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 space-y-2 bg-white">
            ${!response.hasTransaction && !response.hasCanceled ? ` 
