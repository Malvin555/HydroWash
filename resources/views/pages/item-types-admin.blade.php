<x-admin-layout>
    <div class="mb-6 flex items-center justify-between" data-module="itemType">
        <div>
        ...
        <button data-modal-target="modalAddType" data-fetch="false" class="bg-primary text-white p-3 rounded-sm cursor-pointer">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="itemTypesList">

        @if (!$itemType->isEmpty())
            @foreach ($itemType as $item)
                <div class="w-full bg-primary rounded-sm p-3">
                    <img src="{{ $item->image_item ? Storage::url($item->image_item) : '' }}" alt="type item" class="w-full h-60 mb-3">
        ...
        @else
            <div class="w-full bg-primary rounded-sm p-3 flex flex-col items-center justify-center h-80">
                <img src="/img/not-found.png" alt="not found" class="w-32 h-32 opacity-70 mb-4">
                <h1 class="text-white text-lg md:text-xl text-center">
                    No data found
                </h1>
                <p class="text-white text-sm mt-1 text-center opacity-80">Try a different keyword or check your
                    spelling.</p>
            </div>
        @endif
    ...
    <div id="pagination-container">
        {{ $itemType->links('pagination.history-user-pagination') }}
    </div>
