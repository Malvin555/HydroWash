...
<div id="modalAddIroning"
    ...
    <div
        ...
        <div class="overflow-y-auto p-6 flex-1">
            <h2 class="text-xl text-center text-primary font-bold tracking-wide mb-4">Service Service Form</h2>

            <form action="{{ route('ironing-admin.add') }}" method="post" class="space-y-4" id="ironingAdminForm">
                @csrf
                ...
                <div>
                                    ...
                                    @foreach ($chunk as $itemType)
                                        <div class="w-40 relative item-container">
                                            <input type="checkbox" name="selected_types[]" id="item_{{ $itemType->id }}"
                                                ...
                                                {{ session('show_modal') === 'modalAddIroning' && in_array(Str::lower($itemType->name_item), old('selected_types', [])) ? 'checked' : '' }} />
                                            ...
                                            <div
                                                ...
                                                {{ (session('show_modal') === 'modalAddIroning' && in_array(Str::lower($itemType->name_item), old('selected_types', []))) ? 'block' : 'hidden' }}"
                                                >
                                                <div class="flex items-center bg-secondary rounded-sm">
                                                    ...
                                                    <input type="number"
                                                        ...
                                                        value="{{ session('show_modal') === 'modalAddIroning' && old('amounts.' . Str::lower($itemType->name_item)) ? old('amounts.' . Str::lower($itemType->name_item)) : 1 }}">
                                                </div>
                                                @errorIfModal('modalAddIroning', 'amounts.' . Str::lower($itemType->name_item))
                    ...
                    @errorIfModal('modalAddIroning', 'selected_types')
                ...
                <div class="bg-secondary rounded-md p-3 mt-4">
                    ...
                    <input type="hidden" name="total_price" id="totalPriceInput" value="{{ @oldIfModal('modalAddIroning', 'total_price') }}">
                </div>
                ...
                <div class="mb-5">
                    ...
                    <div class="relative inline-block w-full">
                        <select name="retrieval-method" id="retrievalMethod"
                            ...
                            <option value="delivery" class="text-primary" @selectedIfModal('modalAddIroning', 'retrieval-method', 'delivery')>Delivery</option>
                            <option value="take_away" class="text-primary" @selectedIfModal('modalAddIroning', 'retrieval-method', 'take_away')>Take Away
                            </option>
                        </select>
                    ...
                    @errorIfModal('modalAddIroning', 'retrieval-method')
                ...
                <div class="grid grid-cols-1 gap-2" id="addressBox"
                    @if (session('show_modal') === 'modalAddIroning' && old('retrieval-method') === 'delivery')
                        style="display: block;"
                    @endif>
                    ...
                    <div class="flex items-start gap-2 bg-secondary text-primary px-4 py-2 mt-1 rounded-md text-sm">
                        ...
                        <input type="text" name="address" id="address" placeholder="Address"
                            ...
                            value="{{ @oldIfModal('modalAddIroning', 'address') }}" />
                    </div>
                    @errorIfModal('modalAddIroning', 'address')
                    ...
                    <div class="flex items-center gap-2 bg-secondary text-primary px-4 py-2 mt-1 rounded-md text-sm">
                        ...
                        <input type="text" name="destination" placeholder="Destination"
                            class="bg-transparent focus:outline-none w-full placeholder:text-primary"
                            value="{{ @oldIfModal('modalAddIroning', 'destination') }}" />
                    </div>
                    @errorIfModal('modalAddIroning', 'destination')
                ...
                <div class="flex flex-col">
                    ...
                    <textarea name="notes" id="notes" placeholder="Notes"
                        class="bg-secondary text-primary placeholder:text-primary px-4 py-2 w-full h-32 rounded-md resize-none outline-none text-sm">{{ @oldIfModal('modalAddIroning','notes') }}</textarea>
                </div>
