...
<div id="modalEditIroning"
    ...
    <div
        ...
        <x-modal-header title="Ironing Edit Form"></x-modal-header>
        ...
        <div class="modal-data overflow-y-auto p-6 flex-1">
            <h2 class="text-xl text-center text-primary font-bold tracking-wide mb-4">
                {{ $ironing?->name_laundry ?? 'Ironing Edit' }}</h2>

            <form action="{{ route('ironing-admin.update') }}" method="post" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                                    ...
                                    @foreach ($chunk as $itemType)
                                        <div class="w-40 relative item-container-edit">
                                            <input type="checkbox" name="selected_types[]" id="item_{{ $itemType->id }}-edit"
                                                ...
                                                {{ session('show_modal') === 'modalEditIroning' && in_array(Str::lower($itemType->name_item), old('selected_types', [])) ? 'checked' : '' }} />
                                            ...
                                            <div
                                                ...
                                                {{ (session('show_modal') === 'modalEditIroning' && in_array(Str::lower($itemType->name_item), old('selected_types', []))) ? 'block' : 'hidden' }}"
                                                >
                                                <div class="flex items-center bg-secondary rounded-sm">
                                                    ...
                                                    <input type="number"
                                                        ...
                                                        value="{{ session('show_modal') === 'modalEditIroning' && old('amounts.' . Str::lower($itemType->name_item)) ? old('amounts.' . Str::lower($itemType->name_item)) : 1 }}">
                                                </div>
                                                @errorIfModal('modalEditIroning', 'amounts.' . Str::lower($itemType->name_item))
                    ...
                    @errorIfModal('modalEditIroning', 'selected_types')
                ...
                <div class="bg-secondary rounded-md p-3 mt-4">
                    ...
                    <input type="hidden" name="total_price" id="totalPriceInput-edit" value="{{ @oldIfModal('modalEditIroning', 'total_price') }}">
                </div>
                ...
                <div class="mb-5">
                    ...
                    <div class="relative inline-block w-full">
                        <select name="retrieval-method" id="retrievalMethod-edit"
                            ...
                            <option value="delivery" class="text-primary" @selectedIfModal('modalEditIroning', 'retrieval-method', 'delivery')>Delivery</option>
                            <option value="take_away" class="text-primary" @selectedIfModal('modalEditIroning', 'retrieval-method', 'take_away')>Take Away
                            </option>
                        </select>
                    ...
                    @errorIfModal('modalEditIroning', 'retrieval-method')
                ...
                <div class="grid grid-cols-1 gap-2" id="addressBox-edit"
                    @if (session('show_modal') === 'modalEditIroning' && old('retrieval-method') === 'delivery')
                        style="display: block;"
                    @endif>
                    ...
                    <div class="flex items-start gap-2 bg-secondary text-primary px-4 py-2 mt-1 rounded-md text-sm">
                        ...
                        <input type="text" name="address" id="address" placeholder="Address"
                            ...
                            value="{{ @oldIfModal('modalEditIroning', 'address') }}" />
                    </div>
                    @errorIfModal('modalEditIroning', 'address')
                    ...
                    <div class="flex items-center gap-2 bg-secondary text-primary px-4 py-2 mt-1 rounded-md text-sm">
                        ...
                        <input type="text" name="destination" placeholder="Destination"
                            class="bg-transparent focus:outline-none w-full placeholder:text-primary"
                            value="{{ @oldIfModal('modalEditIroning', 'destination') }}" />
                    </div>
                    @errorIfModal('modalEditIroning', 'destination')
                ...
                <div class="flex flex-col">
                    ...
                    <textarea name="notes" id="notes" placeholder="Notes"
                        class="bg-secondary text-primary placeholder:text-primary px-4 py-2 w-full h-32 rounded-md resize-none outline-none text-sm">{{ @oldIfModal('modalEditIroning','notes') }}</textarea>
                </div>
                ...
                <div class="relative inline-block w-full">
                    <select name="status" id="status"
                        ...
                        <option value="pending" class="text-primary" @selectedIfModal('modalEditIroning', 'status', 'pending')>Pending</option>
                        <option value="process" class="text-primary" @selectedIfModal('modalEditIroning', 'status', 'process')>Process</option>
                        <option value="completed" class="text-primary" @selectedIfModal('modalEditIroning', 'status', 'completed')>Completed</option>
                    </select>
                ...
                @errorIfModal('modalEditIroning', 'status')
                ...
                <div class="flex flex-col">
                    ...
                    <input type="date" name="estimation" id="estimation"
                        ...
                        placeholder="Enter estimation" value="{{ @oldIfModal('modalEditIroning', 'estimation') }}">
                    @errorIfModal('modalEditIroning', 'estimation')
                ...
                <div class="relative inline-block w-full mb-0">
                    <div>
                        ...
                        <select name="status-report" id="status-report"
                            ...
                            <option value="normal" class="text-primary" @selectedIfModal('modalEditIroning', 'status-report', 'normal')>Normal</option>
                            <option value="deleted" class="text-primary" @selectedIfModal('modalEditIroning', 'status-report', 'deleted')>Deleted</option>
                        </select>
                    </div>
                ...
                @errorIfModal('modalEditIroning', 'status-report')
