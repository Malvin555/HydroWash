...
<div id="modalEditIroning"
    ...
    <div
        ...
        @php
            if (old('id')) {
                $ironing = DB::table('ironing')
                    ->join('item_types', 'ironing.item_id', '=', 'item_types.id')
                    ->select('ironing.*', 'item_types.image_item', 'item_types.price_item')
                    ->where('ironing.id', old('id'))
                    ->first();
            }
        @endphp
        ...
        <div class="modal-data overflow-y-auto p-6 flex-1">
            <h2 class="text-xl text-center text-primary font-bold tracking-wide mb-4">
                {{$ironing?->name_ironing ?? 'Ironing Edit' }}</h2>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <div class="flex flex-col justify-between">
                            <input type="number" name="amount" id="amount"
                                ...
                                placeholder="Enter amount item" value="{{ old('amount') ?? '0' }}" oninput="updatePriceTotal(this.value)">
                            @error('amount')
